...
export default function TeacherMaterialsScreen() {
  ...
  const fetch = async () => { try { const r = await api.get('/teacher/materials'); setMaterials(r.data); } catch { Alert.alert('Error', 'Failed.'); } finally { setLoading(false); setRefreshing(false); } };
  useEffect(() => { fetch(); }, []);
    ...
    <FlatList data={materials} keyExtractor={i => String(i.id)} contentContainerStyle={s.list}
      ...
      renderItem={({ item }) => (
        <View style={s.card}>
          ...
          {item.fileUrl && <TouchableOpacity style={s.btn} onPress={() => openFile(item.fileUrl!)}><Text style={s.btnText}>Open</Text></TouchableOpacity>}
        </View>
      )}
    />
  );
...
const s = StyleSheet.create({
  ...
  btn: { backgroundColor: '#fef9c3', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10 }, btnText: { fontSize: 12, fontWeight: '800', color: '#d97706' },
