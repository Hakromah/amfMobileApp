...
interface Entry { id: number; day?: string; startTime?: string; endTime?: string; subject?: { name?: string }; classe?: { name?: string }; }
export default function TeacherTimetableScreen() {
  const [entries, setEntries] = useState<Entry[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const fetch = async () => { try { const r = await api.get('/teacher/timetable'); setEntries(r.data); } catch { Alert.alert('Error', 'Failed.'); } finally { setLoading(false); setRefreshing(false); } };
  useEffect(() => { fetch(); }, []);
  ...
  return (
    <FlatList data={entries} keyExtractor={i => String(i.id)} contentContainerStyle={s.list}
      ...
      ListEmptyComponent={<Text style={s.empty}>No timetable found.</Text>}
      renderItem={({ item }) => {
        ...
        return (
          <View style={s.card}>
            ...
            <View style={s.info}><Text style={s.subject}>{item.subject?.name || 'Subject'}</Text>{item.classe?.name && <Text style={s.class_}>{item.classe.name}</Text>}</View>
...
const s = StyleSheet.create({
  ...
  info: { flex: 1 }, subject: { fontSize: 14, fontWeight: '800', color: '#1e293b' }, class_: { fontSize: 11, color: '#64748b', fontWeight: '600', marginTop: 3 },
