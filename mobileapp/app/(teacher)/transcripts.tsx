...
export default function TeacherTranscriptsScreen() {
  ...
  const loadFilterData = async () => {
    ...
    try {
      ...
      console.error('[Teacher Transcripts] load:', error);
  ...
  useEffect(() => {
    const fetchStudentsForClass = async () => {
      ...
      try {
        const res = await api.get(`/teacher/students?classId=${selectedClassId}`);
        setStudents(Array.isArray(res.data) ? res.data : []);
      } catch (err) {
        console.error(err);
      }
  ...
  const fetchStudentTranscripts = async (studentId: number) => {
    ...
    try {
      const res = await api.get(`/teacher/transcripts/student/${studentId}`);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        const res = await api.get(`/teacher/transcripts/${selectedTranscriptId}/preview`);
        ...
        <body>
          <div class="header">
            ...
            <div style="text-align: right">
              <span style="background: #10b981; color: white; padding: 4px 8px; border-radius: 4px; font-weight: bold; font-size: 12px;">Ref: ${meta.referenceNumber}</span>
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View style={s.headerCard}>
              <Text style={s.headerTitle}>Transcripts <Text style={{ color: '#10b981' }}>Matrix.</Text></Text>
              <Text style={s.headerSub}>Manage official academic transcript matrices.</Text>
              ...
              <View style={s.tableContainer}>
                ...
                {transcriptData.courses?.map((c: any, index: number) => (
                  <View key={index} style={s.tbRow}>
                    ...
                    <View style={[s.tbCell, { flex: 1 }]}><Text style={[s.courseVal, { fontWeight: '900', color: '#10b981' }]}>{c.grade}</Text></View>
                  </View>
                ))}
              </View>
...
const s = StyleSheet.create({
  ...
  viewBtn: { backgroundColor: '#10b981', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 10 },
  ...
  downloadBtn: { backgroundColor: '#10b981', paddingVertical: 14, borderRadius: 14, alignItems: 'center' },
